...
class TodoListComponent implements WorkbenchComponent {
  ...
  private todos: Todo[] = [];
  private filter: 'all' | 'active' | 'completed' = 'active';
  private isLoading = false;
  private editId: number | null = null;
  private deleteConfirmId: number | null = null;
  private isAddDrawerOpen = false;
  ...
  private async loadTodos(): Promise<void> {
    ...
    try {
      const completedFilter =
        this.filter === 'all' ? '' : `?completed=${this.filter === 'completed'}`;
      const response = await this.apiRequest<TodoListResponse>(
        `/api/todos${completedFilter}`
      );
  ...
  private async createTodo(title: string, content?: string): Promise<void> {
    if (!title.trim()) return;

    this.setLoading(true);
    try {
      const newTodo = await this.apiRequest<Todo>('/api/todos', {
        method: 'POST',
  ...
  private async toggleTodo(id: number): Promise<void> {
    try {
      const updatedTodo = await this.apiRequest<Todo>(
        `/api/todos/${id}/toggle`,
        { method: 'POST' }
      );
      ...
      if (index !== -1) {
        ...
        if (
          (this.filter === 'active' && updatedTodo.completed) ||
  ...
  private async updateTodo(
    ...
    try {
      const updatedTodo = await this.apiRequest<Todo>(`/api/todos/${id}`, {
        method: 'PATCH',
        body: JSON.stringify({ title, content })
      });
      const index = this.todos.findIndex(t => t.id === id);
  ...
  private async deleteTodo(id: number): Promise<void> {
    try {
      await this.apiRequest<{ success: boolean }>(`/api/todos/${id}`, {
        method: 'DELETE'
      });
  ...
  private setLoading(loading: boolean): void {
    ...
    const inputEl = this.root?.querySelector('.td-input-add') as HTMLInputElement;
  ...
  private startEdit(id: number): void {
    ...
    // 聚焦到编辑框
    setTimeout(() => {
      const editInput = this.root?.querySelector(
        `.td-item[data-id="${id}"] .td-edit-input`
      ) as HTMLInputElement;
      if (editInput) {
        editInput.focus();
        editInput.select();
  ...
  private saveEdit(id: number): void {
    ...
    const editContent = this.root?.querySelector(
      `.td-item[data-id="${id}"] .td-edit-content`
  ...
  private setupEventListeners(): void {
    ...
    drawerInput?.addEventListener('keypress', (e: Event) => {
    ...
    filterBtns.forEach(btn => {
      btn.addEventListener('click', (e) => {
        const filter = (e.currentTarget as HTMLElement).getAttribute(
          'data-filter'
  ...
  private openAddDrawer(): void {
    ...
    const overlay = this.root?.querySelector('#tdDrawerOverlay') as HTMLElement;
    const drawer = this.root?.querySelector('.td-drawer') as HTMLElement;
    const headerAddBtn = this.root?.querySelector('#tdHeaderAddBtn') as HTMLElement;
    ...
    // 聚焦到输入框
    setTimeout(() => {
      const input = this.root?.querySelector('#tdInputAdd') as HTMLInputElement;
  ...
  private closeAddDrawer(): void {
    ...
    // 清空输入
    const input = this.root?.querySelector('#tdInputAdd') as HTMLInputElement;
    const contentInput = this.root?.querySelector('#tdContentAdd') as HTMLTextAreaElement;
    if (input) input.value = '';
    if (contentInput) contentInput.value = '';
  }

  private async handleAddFromDrawer(): Promise<void> {
    const input = this.root?.querySelector('#tdInputAdd') as HTMLInputElement;
    const contentInput = this.root?.querySelector('#tdContentAdd') as HTMLTextAreaElement;
  ...
  private showDeleteConfirm(id: number, itemElement: HTMLElement): void {
    // 关闭之前的确认框
    this.cancelDeleteConfirm();
  ...
  private renderTodoList(): void {
    const listEl = this.root?.querySelector('#tdList') as HTMLElement;
    const emptyEl = this.root?.querySelector('#tdEmpty') as HTMLElement;
    const statTotal = this.root?.querySelector('#tdStatTotal') as HTMLElement;
    const statActive = this.root?.querySelector('#tdStatActive') as HTMLElement;
    ...
    listEl.innerHTML = this.todos
      .map(
        todo => `
      <div class="td-item ${todo.completed ? 'td-item--completed' : ''} ${
          this.editId === todo.id ? 'td-item--editing' : ''
        }" data-id="${todo.id}">
        ${this.editId === todo.id
  ...
  private renderViewMode(todo: Todo): string {
...
✕
  ...
  private renderEditMode(todo: Todo): string {
    return `
      <input
        type="text"
        class="td-edit-input"
  ...
  private bindTodoItemEvents(): void {
    if (!this.root) return;
    ...
    items.forEach(item => {
      ...
      deleteBtn?.addEventListener('click', () => {
        ...
        if (this.deleteConfirmId === id) {
          ...
          // 显示确认框
          this.showDeleteConfirm(id, item as HTMLElement);
        }
      });
      ...
      editInput?.addEventListener('keypress', (e: Event) => {
        const keyEvent = e as KeyboardEvent;
